...
export default function StatCard({ label = '', value = 0, trend = null, color = 'blue', icon = null }) {
  const colorClasses = {
    ...
    <div className={`border ${colorClasses[color] || colorClasses.blue} p-6 rounded-lg`}>
      <div>
        ...
        {trend !== null && trend !== undefined && (
          <p className={`text-xs mt-2 ${trend > 0 ? 'text-green-600' : trend < 0 ? 'text-red-600' : 'text-gray-500'}`}>
